...
class GPAlgorithm:
    ...
    def loadData(self):
        ...
        with open(self.filename, "r") as f:
            ...
            for line in f.readlines()[:100]:
                ...
                self.input.append(values[0:-1])#[1:-1])
                self.output.append(values[-1])
                print("input",self.input)
                print("output",self.output)
                self.n += 1
        self.input=[[y/10 for y in x] for x in self.input]
        shuffle(self.input)
        shuffle(self.output)
    ...
    def iteration(self, i):
        ...
        for i in range(nrChildren):
            offspring.individuals[i] = Chromosome.crossover(self.population.individuals[i],
